...
#define APEX_OFF_LOCAL_PLAYER            0x03D73118ULL  // RE-PINNED 2026-05-14 via wide .text static xref scan (UNICORN_DUMPER + r5apex_live_text 5/14 13:25). Old 0x0268FA08 had 0 xrefs (slot dead). New slot has 98 xrefs incl. 9 direct entity-field ops on +0x324 (m_iHealth): canonical pattern mov rax,[rip+CAND]; mov [rax+0x324],<val> (health write/death) + cmp [rax+0x324],3 (life check). 89 of 98 xrefs pass as this to member fns. Distinct from VIEW_RENDER (0x03D9A008, -0x26EF0). Symptom that triggered re-pin: HasLocal=false across 13 drains, EL0 never emitted, 348 DR0 ticks in active gameplay yielded LPV: 0x0 every read.
